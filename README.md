...
📦 project-root/
├── app.py                       # Flask 서버 진입점
├── rag/                         # 🔍 RAG(유사도 검색) 패키지 (feature/RAG)
│   ├── __init__.py
│   └── embedder.py             # OpenAI 임베딩 생성 유틸 (text-embedding-3-small)
├── scripts/                     # 🛠️ 유틸리티 스크립트
│   └── build_embeddings.py     # products.json 기반 임베딩 인덱스 빌드
├── static/
│   ├── css/                     # 스타일 시트들 (style.css, chat.css 등)
│   ├── js/                      # 클라이언트 JS (result.js, logger.js 등)
│   └── data-json/              # 📁 JSON 데이터 폴더
│       ├── products.json       # 추천 제품 데이터 (질의 문자열 키 구조)
│       ├── embeddings.json     # 임베딩 인덱스 (질의 → 1536차원 벡터)
...
- [ ] 파트너 제휴 링크 시스템 고도화
- [🔄] **RAG 기반 유사도 검색 도입** (feature/RAG 진행 중)
  - Phase 1 ✅ 임베딩 유틸 (`rag/embedder.py`)
  - Phase 2 ✅ 임베딩 인덱스 빌더 (`scripts/build_embeddings.py`)
  - Phase 3 ⏳ 유사도 검색 엔진 (`rag/retriever.py`)
  - Phase 4 ⏳ `/api/products` exact-match → RAG 검색 교체
...
- 현재 `/api/products`는 사용자 질의와 JSON 키가 **완전히 일치해야만** 캐시가 동작하는 구조 → "운동용 이어폰"과 "운동할 때 쓸 이어폰"이 다른 질의로 처리됨.
- 이를 해결하기 위해 `feature/RAG` 브랜치에서 **OpenAI 임베딩(text-embedding-3-small) 기반 유사도 검색**을 도입 중. 의미상 유사한 질의는 기존 캐시를 재사용하고, 완전히 새로운 질의만 GPT를 호출하는 구조로 리팩토링 예정.
